    ...
    *,
    ...
    payload = {
        ...
        "rules": [
            ...
            "role_entity 专指凭证所属的核算主体/记账主体，例如公司代码、公司名称、账套公司、法人实体、business unit、company code。",
            "role_entity 绝不是交易对手方或往来单位；不得选择客户、供应商、客商、收付款对象、对方单位、对方户名等字段。即使对手方样例是公司名称，也不能映射为 role_entity。",
            "没有明确核算主体列时，role_entity 应保持缺失，不得用交易对手方字段凑数。",
    ...
    *,
    ...
    payload = {
        ...
        "rules": [
            ...
            "current_year_dep means annual/YTD depreciation only, such as 本年折旧, 当年折旧, 本年至今折旧, or 年折旧额.",
            "current_year_dep must NOT map to monthly depreciation columns such as 本月折旧, 当月折旧, or 月折旧; if only monthly depreciation exists, leave current_year_dep unmapped or do not suggest a replacement.",
    ...
    *,
    ...
    payload = {
        ...
        "rules": [
            ...
            "Your task is NOT to find the most unique or strongest match key.",
            "Your task is ONLY to check whether current_match is equivalent to reference_match from step 1.",
            ...
            "If current_match already has the same number of columns and the same business roles as reference_match, return status=ok, action=keep, and echo the current columns.",
            "Do not suggest a replacement merely because another column could improve uniqueness or reduce duplicates.",
            "The suggested_fileX_columns count MUST equal the corresponding reference_match.fileX count.",
            "Never add extra ID columns beyond reference_match.",
            ...
            "Example: reference_match = code + name; current_match = code + name; even if category/type/model columns exist, the correct output is keep, not category + code + name.",
