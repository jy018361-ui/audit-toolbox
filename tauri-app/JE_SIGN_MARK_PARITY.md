...
## 2026-09-23：沿用统一 JE 映射门禁

- 科目编码与科目名称任一映射即可，摘要为选填；凭证识别字段、日期及金额方案仍按看账公共规则校验。
- 正负数标记与看账继续共用 `missingKanzhangRequiredRoles`，避免两个 JE 单表工具出现不同门禁。

## 2026-09-22 · 目标科目搜索选择边界修复

- 目标科目面板不再沿用普通列漏斗“空筛选即全选”的初始状态；空批次首次打开时保持未选。
- 搜索后点击“全选”只加入当前搜索结果，不会把搜索前加载的全量科目作为隐藏选中项一并提交；用户此前显式勾选的跨搜索结果仍会保留。
- 回归：`npx vitest run src/components/ColumnFilterMenu.test.tsx src/JeSignMarkPage.test.tsx`。
