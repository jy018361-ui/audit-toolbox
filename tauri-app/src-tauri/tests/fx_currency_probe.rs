...
fn probe_account_currencies() {
    ...
    for (name, method) in [
        ...
        let details = v["accountCurrencyDetails"].as_object().cloned().unwrap_or_default();
        let mut by_source = std::collections::BTreeMap::<String, usize>::new();
        for (_, d) in &details {
            *by_source.entry(d["source"].as_str().unwrap_or("?").into()).or_default() += 1;
        }
        println!("\n══════ {name}  科目数 {}  依据分布 {:?}", details.len(), by_source);
        // 重点看「退回本位币列」的——这些就是需要人工指定的
        let mut shown = 0;
        for (account, d) in &details {
            if d["needsConfirmation"].as_bool().unwrap_or(false) && shown < 8 {
                println!("  需确认: {account} -> {} (seen {})", d["detected"], d["seen"]);
                shown += 1;
            }
        }
        // 多币种科目
        ...
        for (account, d) in &details {
            ...
            if n > 1 && multi < 5 {
                println!("  多币种: {account} -> 主 {} / 全部 {}", d["detected"], d["seen"]);
                multi += 1;
            }
        }
        // 截图里那两个银行科目
        for (account, d) in &details {
            if account.contains("10020002") || account.contains("2241170003") || account.contains("1002990001") {
                println!("  样例: {account} -> {} 依据 {} seen {}", d["detected"], d["source"], d["seen"]);
            }
        }
    }
}
...
fn probe_account_name_alignment() {
    ...
    for (tb_name, je_name) in [
        ...
        let tb_codes: std::collections::BTreeSet<_> =
            tb_accounts.iter().map(|a| code(a)).collect();
        let je_codes: std::collections::BTreeSet<_> =
            je_accounts.iter().map(|a| code(a)).collect();
