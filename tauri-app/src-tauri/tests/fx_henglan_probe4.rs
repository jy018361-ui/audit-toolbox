...
fn inspect(method: &str, path: &str) -> Value {
    audit_toolbox_lib::engine_call_for_test(method, json!({"source": {"inputPath": path}}))
        .expect("inspect 应当成功")
}
...
fn probe_golden_currency_rule() {
    ...
    println!(
        "金标 TB currency 建议 = {}",
        serde_json::to_string(&tb["suggestedMapping"]["currency"]).unwrap()
    );
    println!(
        "金标 TB currencyText 建议 = {}",
        serde_json::to_string(&tb["suggestedMapping"]["currencyText"]).unwrap()
    );
    println!(
        "金标 JE currency 建议 = {}",
        serde_json::to_string(&je["suggestedMapping"]["currency"]).unwrap()
    );
    let mut params = json!({
        "mode": "combined",
        "fixedEntity": "",
