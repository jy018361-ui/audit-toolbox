...
const hooks = vi.hoisted(() => ({
  listeners: [] as Array<(event: JobEvent) => void>,
}));
      ...
      matches: [
        {
          ...
          level: "auto",
          total: 96,
          breakdown: { charSim: 1, lcsSim: 1, tokenOverlap: 1 },
          reasons: ["清洗后完全一致"],
        },
      ],
    },
      ...
      matches: [
        {
          ...
          level: "suspect",
          total: 78,
          breakdown: { charSim: 0.78, lcsSim: 0.8, tokenOverlap: 0.7 },
          reasons: ["疑似简称差异"],
        },
        {
          bIndex: 3,
          ...
          level: "suspect",
          total: 72,
          breakdown: { charSim: 0.72, lcsSim: 0.74, tokenOverlap: 0.6 },
          reasons: ["疑似简称差异", "组织形式不同"],
        },
      ],
    },
...
async function setupBothSources() {
  ...
  vi.mocked(pickPath).mockImplementation(
    async (_kind: unknown, title: string) =>
  ...
  await waitFor(() =>
    expect(screen.getByText("来源 A匹配列")).toBeInTheDocument(),
  );
...
describe("FuzzyMatchPage", () => {
  ...
  it("completed 事件后渲染总览四个指标与明细", async () => {
    ...
    expect(screen.getAllByText("北京甲有限公司").length).toBeGreaterThanOrEqual(
      2,
    );
  ...
  it("疑似确认卡：点候选后置灰、进度 +1、实时保存确认", async () => {
    ...
    const cardA = screen
      .getAllByText("上海乙股份")
      .find((el) => el.closest(".fuzzy-confirm-card"));
    expect(cardA).toBeTruthy();
    const card = cardA!.closest(".fuzzy-confirm-card")!;
    expect(
      screen.getByRole("button", { name: /都不是（拒绝匹配）/ }),
    ).toBeInTheDocument();
    ...
    const { engineCall } = await import("./api");
    await waitFor(() =>
      expect(engineCall).toHaveBeenCalledWith("fuzzy.save_confirm", {
        jobId: "job-1",
        confirmations: [{ aIndex: 1, bIndex: 2, action: "accept" }],
      }),
    );
    // 确认草稿落 sessionStorage，重挂载后可接续。
    expect(sessionStorage.getItem("fuzzy-match-draft.v1")).toContain(
      '"aIndex":1',
    );
  });
});
