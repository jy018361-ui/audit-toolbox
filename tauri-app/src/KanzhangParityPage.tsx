...
export const KZ_ROLE_LABELS:Record<keyof Mapping,string>={id:"凭证编号",account:"科目名称",entity:"公司/核算主体",date:"日期",summary:"摘要",amount:"方案A-金额",direction:"方案A-方向",debit:"方案B-借方",credit:"方案B-贷方"};
const isMultiRole=(role:keyof Mapping)=>role==="id"||role==="account";
// 与 Rust `validate_kanzhang_mapping` 保持同一必填口径：凭证 ID、科目，
// 以及方案 A 的金额列或方案 B 的借贷两列。方向列是方案 A 的选填项。
export function missingKanzhangRequiredRoles(mapping:Mapping):string[]{
  const missing:string[]=[];
  if(!mapping.id.some(value=>Boolean(value?.trim())))missing.push("唯一识别码 (ID)");
  if(!mapping.account.some(value=>Boolean(value?.trim())))missing.push("科目名称");
  const hasAmount=Boolean(mapping.amount?.trim());
  const hasDebitCredit=Boolean(mapping.debit?.trim()&&mapping.credit?.trim());
