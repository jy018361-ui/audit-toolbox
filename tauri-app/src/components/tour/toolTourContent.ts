...
export const TOOL_TOUR_SCRIPTS: Record<string, ToolTourScript> = {
  fa_list: {
    ...
    prepare:
      "年初、年末两份固定资产清单（Excel/CSV）。两份需选得出相同的资产ID（可多列组合），并映射到资产类别、资产名称、原值、累计折旧等必填字段。",
    ...
    stepHints: {
      "1": "分别放入年初、年末清单，在预览表头的下拉里核对资产ID（可多列）和必填字段，然后执行匹配——匹配是后面所有步骤的地基。",
  ...
  fa_policy_compare: {
    ...
    prepare:
      "年初、年末两份固定资产清单（Excel/CSV）。必填字段：资产类别、资产名称、原值、累计折旧；两份要选得出相同的资产ID（可多列组合）。",
    prepareTargeted: true,
  ...
  meeting_minutes: {
    purpose:
      "开会时自动检测 Teams 会议并询问是否记录；录音上传阿里云百炼转写并区分说话人，再由统一大模型整理成结构化纪要，也可导入已有录音补生成。",
    prepare:
      "先在设置页「语音转写（百炼）」配置 API 密钥并确认统一 LLM 已启用；保持工具箱开着，加入 Teams 会议即可。",
    prepareTargeted: true,
    flow: "三步：加入会议后确认开始记录（或手动开始/导入录音）→ 自动转写并生成纪要 → 打开纪要与转写稿。",
    result:
      "同一会议文件夹内的「会议纪要.md」与「转写稿.txt」，纪要含会议信息、讨论要点、会议结论与待办事项表格；简要/标准/详细三档可从转写稿重新生成，无需重新上传录音。",
    stepHints: {
      "1": "检测到会议会弹窗询问；也可在本页手动「开始记录」，或「浏览」导入已有录音文件（wav/mp3/m4a 等）。想关窗后继续监控，打开「后台常驻」，窗口会缩到系统托盘。",
      "2": "确认前注意提示：录音将上传阿里云百炼进行转写，请先告知参会人。",
      "3": "生成完成后可直接打开纪要与转写稿；对详细度不满意可换档位用「从转写稿重新生成」。",
    },
  },
